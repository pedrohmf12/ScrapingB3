...
from openpyxl.styles import Font, Border, Side, PatternFill, Font, GradientFill, Alignment,Color
from time import sleep
...
def get_key(dictionary, value):
    """Retorna a chave correspondente a um valor em um dicionário"""
    for key in dictionary:
        if dictionary[key] == value:
            return key

#Representação das siglas que representam os meses do ativo BGI
ativos = {
    "Março": "H",
    "Maio": "K",
    "Julho": "N",
...
siglas = list(ativos.values())

meses = list(ativos.keys())

preco_ativo=[]
for sigla in siglas:
    driver.get(f"https://br.tradingview.com/symbols/BMFBOVESPA-CCM{sigla}2023/")
    sleep(2)
...
planilha = df.to_excel('Contrato_Futuro_Milho.xlsx', index=False)

#importando DataFrama para excel
file_name = 'Contrato_Futuro_Milho.xlsx'
...
for i in variations_perc: 
    valores = ws[f'{i}'].value
    fixed_string = valores.replace('(','').replace(')','').replace('+','').replace("−", "-")
    #float_value = float(fixed_string)
...
for i in range(5):
    ...
    for cell in cells:
        ws[f'{cell}{i+2}'].alignment = Alignment(horizontal= 'center', vertical='center')
    for cell in cells:
        if i%2 == 0:
            ws[f'{cell}{i+2}'].fill = my_fill_zebra
        else:
            ws[f'{cell}{i+2}'].fill = my_fill
    for cell in bordas:
        ws[f'{cell}{i+2}'].border = borders
    for cell in moedas:
        ws[f'{cell}{i+2}'].number_format = 'R$ #,##0.00;[Red]-R$ #,##0.00'
    for cell in cells[3]:
        if ws[f'{cell}{i+2}'].value < str('0'):
            font = Font(color=Color(rgb='FF0000'))
            ws[f'{cell}{i+2}'].font = font
            ws[f'{cell}{i+2}'].number_format = '+#,##0.00%;[Red]-#,##0.00%'
        else:
            ws[f'{cell}{i+2}'].number_format = '+#,##0.00%;-#,##0.00%'
...
image_path = os.path.join(os.getcwd(),'planilha2.bmp')

# Salve a imagem em alta resolução
image.save(image_path, quality=100)

image = Image.open("planilha2.bmp")

# Salve a imagem como um arquivo PNG
image.save(".\imagens\cotacao_milho.png", "PNG")

os.remove("planilha2.bmp")
